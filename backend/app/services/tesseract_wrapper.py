...
class TesseractWrapper:
    ...
    def extract_text_safe(
        ...
        try:
            ...
            logger.warning(f"Direct extraction failed: {e}, trying subprocess method...")
            self._error_count += 1
    ...
    def _extract_subprocess(self, image_path: str, config_str: str, lang: str) -> str:
        ...
        try:
            ...
            env['TESSDATA_PREFIX'] = '/opt/homebrew/share/tessdata/'  # Ensure tessdata path
            ...
            if result.returncode != 0:
                logger.warning(f"Tesseract subprocess returned {result.returncode}: {result.stderr}")
    ...
    def extract_with_confidence_safe(
        ...
        try:
            ...
            text, conf_dict = self._extract_with_confidence_direct(image_path, config_str, lang)
            if text and len(text.strip()) > 0:
                self._error_count = 0
...
def extract_text_with_confidence_resilient(
    ...
    """
    wrapper = get_tesseract_wrapper()
    text, conf_dict, success = wrapper.extract_with_confidence_safe(image_path, config_str, lang)
