...
def detect_text_regions_mser(image: np.ndarray, min_area: int = 50) -> list[tuple[int, int, int, int]]:
    """
...
def merge_overlapping_boxes(
    ...
    for i in range(len(boxes_array)):
        ...
        for j in range(i + 1, len(boxes_array)):
            ...
            area2 = (boxes_array[j][2] - boxes_array[j][0]) * (boxes_array[j][3] - boxes_array[j][1])
        ...
        for i, (x, y, w, h) in enumerate(regions):
            ...
            try:
                ...
                if region_text and region_text.strip():
                    region_texts.append((y, region_text))  # Store with y-coordinate for sorting
            except Exception as e:
...
async def extract_parallel_strategies(
    ...
    def strategy_advanced():
        return extract_with_multiple_strategies(filepath, document_type, max_strategies=3)
        ...
        if not isinstance(result, Exception):
            if isinstance(result, tuple) and len(result) == 2:
                ...
                if isinstance(metadata, dict):
                    conf = metadata.get('confidence', 0) or \
                           metadata.get('original_confidence', {}).get('average_confidence', 0)
                else:
                    conf = 50.0
