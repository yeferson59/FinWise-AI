...
def assess_image_quality(image_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        is_acceptable = (
            ...
            resolution_ok                  # Minimum resolution
        )
        
        # 6. Generate recommendations
        recommendations = []
        if blur_score <= 100:
            recommendations.append("Image is too blurry - use sharper image or stabilize camera")
        if brightness <= 50:
            recommendations.append("Image too dark - increase lighting or exposure")
        elif brightness >= 200:
            recommendations.append("Image overexposed - reduce brightness or use better lighting")
        if contrast <= 30:
            recommendations.append("Low contrast - improve lighting conditions")
        if not resolution_ok:
            recommendations.append(f"Resolution too low ({w}x{h}) - use higher resolution (min 300x300)")
...
def auto_correct_image_from_path(image_path: str, quality_info: dict[str, Any] | None = None) -> np.ndarray:
    """
