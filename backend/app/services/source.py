...
async def update_source(session: SessionDep, id: int, update_source: UpdateSource):
...
async def classify_source_from_text(
    ...
    """
    source_count = db.get_total_count(Source, session)

    if source_count == 0:
        raise ValueError(
            "No sources found in the database. Please create sources before classifying documents."
        )

    text = text.strip()
    if not text:
        raise ValueError("Text cannot be empty for classification")
    ...
    # Fetch current sources from the database to validate the agent's output later
    sources = await _source_crud.get_all(session, 0, max(100, source_count))
    source_names = [s.name for s in sources]

    prompt = (
        "You are an expert document source identification assistant. Your task is to compare the provided text "
        "against the existing sources and determine the best fit. Use any available tools to retrieve "
        "the exact list of sources from the database. Then, for each source, provide a short comparison "
        "to the text and a numeric confidence score from 0 to 100 indicating how well the text matches that "
        "source. The comparison helps ensure you actually considered the text against each source.\n\n"
        "Important output requirements:\n"
        "1) For each source, show a one-line entry with the source name, a score (0-100), and a one-sentence reason.\n"
        "2) On the very last non-empty line of your output, return ONLY the exact name of the single source that "
        ...
        "Follow the steps: (a) retrieve sources using the tools, (b) compare each source to the text and give a score, "
        "(c) choose the best source and put its exact name on the final line only. Begin."
    )
    ...
    # Extract the final non-empty line as the candidate source name
    candidate_name = None
    ...
    # Validate candidate against existing source names (exact match)
    matched_name = None
    if candidate_name and candidate_name in source_names:
        matched_name = candidate_name
    else:
        # Try case-insensitive exact match
        lower_to_name = {n.lower(): n for n in source_names}
        if candidate_name and candidate_name.lower() in lower_to_name:
            matched_name = lower_to_name[candidate_name.lower()]
        else:
            # As a fallback, try to find any source name that appears somewhere in the agent output
            found = None
    ...
    if not matched_name:
        raise ValueError(
            "Unable to classify the document source: the agent's output did not match any existing source.\n"
            f"Agent output:\n{response_text}"
        )

    source = db.get_entity_by_field(Source, "name", matched_name, session)
    if not source:
        raise ValueError(
            f"The classified source '{matched_name}' does not exist in the database."
        )
