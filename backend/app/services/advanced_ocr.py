        ...
        try:
            ...
            if image is not None:
                ...
                if rotation != 0:  # Only if rotation was applied
                    ...
                    results.append({
                        'text': text2,
                        'confidence': avg_conf * 1.05,  # Bonus for orientation correction
        ...
        try:
            ...
            if not quality_info['is_acceptable']:
                ...
                if image is not None:
                    ...
                    temp_fd, temp_path = tempfile.mkstemp(suffix='.png', prefix='corrected_')
                    os.close(temp_fd)
                    ...
                    results.append({
                        'text': text3,
                        'confidence': avg_conf * 1.1,  # Bonus for quality correction
...
def analyze_results(results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        'agreement_level': 'high' if max(confidences) - min(confidences) < 20 else 'low'
    }
