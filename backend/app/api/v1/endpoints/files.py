    ...
    **Performance**: 90-98% accuracy (vs 70-85% baseline)
    ...
    try:
        ...
        if is_poor_quality:
            ...
            import tempfile
            temp_fd, temp_corrected = tempfile.mkstemp(suffix='.png', prefix='corrected_')
            import os
            os.close(temp_fd)
        ...
        try:
            if is_large:
                ...
                print(f"📏 Large image ({width}x{height}), using incremental processing...")
                ...
                try:
                    ...
                    avg_conf = metadata.get('original_confidence', {}).get('average_confidence', 100)
                    if avg_conf < 75 and not is_large:
                        print(f"📊 Low confidence ({avg_conf:.1f}%), upgrading to advanced strategies...")
            ...
            print(f"All advanced strategies failed: {inner_e}, using basic extraction...")
            processed_path = preprocessing.preprocess_image(
                local_path, document_type=doc_type, save_to_temp=True
            )
...
async def clear_ocr_cache(max_age_days: Annotated[int, Query(description="Clear cache older than N days", ge=1)] = 7):
...
async def assess_image_quality_endpoint(file: UploadFile):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error assessing image quality: {str(e)}")
    finally:
...
async def extract_text_advanced(
    document_type: Annotated[
        str | None,
        Query(description="Type of document: receipt, invoice, document, form, screenshot, photo, general"),
    ],
    file: UploadFile,
):
    """
...
async def extract_text_optimized(
    ...
    mode: Annotated[
        str,
        Query(description="Optimization mode: 'parallel' (fastest), 'regions' (sparse text), 'incremental' (large images)")
    ] = "parallel",
